...
class ViT(nn.Module):
    def __init__(self, d_model, n_patches, n_encoders, n_heads, patch_res) -> None:
        ...
        self.register_buffer('pos_embed', self.__positional_encoding(n_patches ** 2 + 1, d_model))
        self.encoders =  nn.ModuleList([ViTEncoder(d_model, n_heads) for _ in range(n_encoders)])
        self.final_linear = nn.Linear(d_model, 10)

    def forward(self, x):
        linear_maps = self.linear_embed(x)
        inputs = torch.cat((self.class_token.expand(x.shape[0], 1, -1), linear_maps), dim=1)
    ...
    def __positional_encoding(self, max_seq_length, d_model):
        ...
        for seq_index in range(0, max_seq_length):
            for index in range(0, d_model // 2):
                denom = torch.Tensor(
                    [seq_index / (10000 ** ((2 * index) / d_model))]
                )
                (
